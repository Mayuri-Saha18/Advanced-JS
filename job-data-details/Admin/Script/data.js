...
name.addEventListener('keyup', async (e) => {
  ...
  await fetch(
    `https://data-server-seven.vercel.app/users?q=${e.target.value}`
  )
    .then((res) => res.json())
...
const getUsers = async () => {
  // console.log('hello');
  let data = await fetch('https://data-server-seven.vercel.app/users')
    .then((res) => res.json())
...
const deleteUser = async (user) => {
  await fetch(`https://data-server-seven.vercel.app/users/${user.id}`, {
    method: 'DELETE',
